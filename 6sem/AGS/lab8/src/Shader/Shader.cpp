...
void Shader::SetUniform(std::string name, const std::vector<glm::mat4> &value)
{
    // 1. Защита от пустого массива (предотвращает краш памяти C++)
    if (value.empty())
        return;

    GLuint loc = GetUniformLocation(name);

    // 2. Жестко ограничиваем размер, чтобы драйвер Mesa не сошел с ума
    int count = value.size();
    if (count > 20)
        count = 20;

    // Отправляем данные в видеокарту безопасно
