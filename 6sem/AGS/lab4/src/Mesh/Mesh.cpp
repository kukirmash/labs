...
//--------------------------------------------------------------------------
// Парсер .obj файлов
bool Mesh::ParseOBJ(const std::string &filename, std::vector<Vertex> &out_vertices, std::vector<GLuint> &out_indices)
{
        ...
        else if (type == "f")
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (uniqueVertices.count(vertexData[i]) == 0)
                {
                    ...
                    vertex.coord[0] = temp_positions[vIdx - 1].x;
                    vertex.coord[1] = temp_positions[vIdx - 1].y;
                    vertex.coord[2] = temp_positions[vIdx - 1].z;

                    // Текстурные координаты (если они есть в файле)
                    if (!temp_texcoords.empty() && vtIdx > 0)
                    {
                        vertex.texCoord[0] = temp_texcoords[vtIdx - 1].x;
                        vertex.texCoord[1] = temp_texcoords[vtIdx - 1].y;
                    }
                    else
                    {
                    ...
                    // Нормали
                    if (!temp_normals.empty() && vnIdx > 0)
                    {
                        vertex.normal[0] = temp_normals[vnIdx - 1].x;
                        vertex.normal[1] = temp_normals[vnIdx - 1].y;
                        vertex.normal[2] = temp_normals[vnIdx - 1].z;
                    }
                    else
                    {
                    ...
                    GLuint newIndex = static_cast<GLuint>(out_vertices.size() - 1);
                ...
                else
                {
                    ...
                    out_indices.push_back(uniqueVertices[vertexData[i]]);
                }
            }
        }
    }
